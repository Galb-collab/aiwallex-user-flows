      ...
      <div className="revolut-step">
        ...
        <div className="revolut-add-money-options">
          <button type="button" className="revolut-add-option" onClick={handleRevtagClick}>
            ...
            <div className="revolut-add-option-content">
          ...
          <button type="button" className="revolut-add-option" onClick={() => navigate(flowPath('/flow/revolut-exchange'))}>
            ...
            <span className="revolut-add-info" title="More information">ℹ</span>
          </button>
        </div>
      </div>
      ...
      {showRevtagModal && (
        <div className="revolut-modal-overlay" onClick={() => setShowRevtagModal(false)}>
          <div className="revolut-modal revolut-revtag-modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="revolut-modal-subtitle">Hey! You can now send money to other Revolut accounts and Revolut Business accounts using Revtag. No details required. Here&apos;s mine:</p>
            <p className="revolut-revtag-handle"><strong>@mobbinnpc4</strong></p>
            <h3 className="revolut-revtag-share-title">Share</h3>
            <div className="revolut-revtag-actions">
              <button type="button" className={`revolut-revtag-copy ${linkCopied ? 'copied' : ''}`} onClick={handleCopyLink}>
                <span>📋</span> Copy link {linkCopied && <span className="revolut-link-copied">✓ Link copied</span>}
              </button>
              <button type="button" className="revolut-revtag-send">✉ Send</button>
            </div>
          </div>
        </div>
      )}
